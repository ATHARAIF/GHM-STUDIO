...
func _generate_grid():
	...
	# Nyari Mesh dan ngambil Transform aslinya dari GLB[cite: 5]
	for child in temp_scene.get_children():
		if child is MeshInstance3D:
			mesh_target = child.mesh
			original_transform = child.transform
			break
		# Jaga-jaga kalau dari Blender nge-group nodenya lebih dalem[cite: 5]
	...
	# Kalau mesh dan proporsi asli udah ketemu, susun multimesh-nya[cite: 5]
	if mesh_target:
