...
@onready var grid: GridContainer = $PopupUI/CenterContainer/PanelContainer/VBoxContainer/GridContainer
...
func _ready() -> void:
	...
	grid = $PopupUI/CenterContainer/PanelContainer/VBoxContainer/GridContainer
...
func _build_ui() -> void:
	...
	# Farm Environment
	_add_row("Luas Lahan", "%.1f Ha" % location.surface_area, Color(0.2, 0.8, 0.2))
	_add_row("Orientasi Kebun", location.exposure, Color(0.8, 0.8, 0.2))
	...
	_add_row("Ketinggian (mdpl)", "%d mdpl" % location.altitude, alt_color)
	...
	_add_row("pH Tanah", "%.1f" % location.ph_level, ph_color)
	...
	_add_row("Kemiringan Lahan", "%.0f%%" % location.slope, slope_color)
	...
	_add_row("Kadar Pasir", "%.0f%%" % location.soil_sand, sand_color)
	...
	_add_row("Kadar Tanah Liat", "%.0f%%" % location.soil_clay, clay_color)
	
	# Loam is the rest
	_add_row("Kadar Lempung", "%.0f%%" % location.soil_loam, Color(0.2, 0.8, 0.2))
	
	# Tree Stats
	if tree:
		_add_row("Umur Tanaman", "%.0f Tahun" % tree.age_years, Color(0.8, 0.2, 0.2) if tree.age_years < 3 else Color(0.2, 0.8, 0.2))
		_add_row("Varietas Kopi", var_data.variety_name if var_data else "Unknown", Color(0.2, 0.8, 0.8))
		...
		_add_row("Kepadatan Tanam", "%d Pohon/Ha" % tree.planting_density, den_color)
		
		_add_row("Kesehatan Pohon", "%.0f%%" % tree.health_pct, Color(0.8, 0.8, 0.2))
		_add_row("Kematangan Ceri", "%.0f%% Brix" % tree.current_brix, Color(0.8, 0.2, 0.2))
		_add_row("Keasaman", "%.0f / 100" % tree.current_acidity, Color(0.2, 0.8, 0.2))
