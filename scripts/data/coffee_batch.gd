...
## Catatan bebas dari player tentang batch ini.
@export var note: String = ""

## Skor cupping keseluruhan, dihitung otomatis dari rata-rata attribute.
func get_score() -> float:
	var total = aroma + acidity + body + sweetness + flavor + bitterness
	return total / 6.0
...
func apply_effects(tile_dict: Dictionary) -> void:
	...
	cherry_kg = int(clamp(float(cherry_kg) * (1.0 + card_data.effect_yield), 0, 5000))
