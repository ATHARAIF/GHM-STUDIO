...
@onready var layer_calendar = $menu_calendar

# Close Buttons (sesuaikan path node-nya dengan struktur scene kamu)
@onready var btn_close_menu = $menu_tabs/header/tabs/exit

func _ready() -> void:
	StageManager.turn_changed.connect(_on_turn_changed)
	StageManager.budget_changed.connect(_on_budget_changed)
	StageManager.turn_changed.connect(_on_calendar_turn_changed)
	...
	if btn_close_menu:
		btn_close_menu.pressed.connect(func(): _close_popup(layer_menu_tabs))
...
func _close_popup(layer: CanvasLayer) -> void:
	if layer:
		layer.visible = false

func _on_calendar_pressed() -> void:
	if not layer_calendar:
		return
	if layer_calendar.visible:
		layer_calendar.visible = false
	else:
		layer_calendar.visible = true
		if layer_menu_tabs:
			layer_menu_tabs.visible = false
		if layer_coffee_log:
			layer_coffee_log.visible = false
		if layer_calendar.has_method("refresh"):
			layer_calendar.refresh()

func _on_calendar_turn_changed(_turn: int, _season: int, _year: int) -> void:
	if layer_calendar and layer_calendar.visible and layer_calendar.has_method("refresh"):
		layer_calendar.refresh()
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and event.keycode == KEY_P:
		for location in StageManager.farm_locations:
			if not location.owned:
				StageManager.unlock_farm_location(location)
				break
