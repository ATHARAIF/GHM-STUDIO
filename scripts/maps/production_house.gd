...
func _spawn_factory_machines() -> void:
	for m_id in FactoryManager.placed_machines:
		...
		if item.tile_scene:
			...
			var can_place_at = func(c: Vector2i) -> bool:
				for offset in shape:
					var check_c = c + offset
					if not PlacementManager.grid.has(check_c) or PlacementManager.grid[check_c].is_occupied:
						return false
				return true
				
			if can_place_at.call(coord):
